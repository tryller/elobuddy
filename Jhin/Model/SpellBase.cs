...
namespace Jhin.Model
{
    ...
    public class SpellBase
    {
        ...
        public GameObject RangeCheckSourceObject = Program.MyHero;

        public Slider Slider;
        public SpellSlot Slot;

        public GameObject SourceObject = Program.MyHero;
        ...
        {
            get { return Slot != SpellSlot.Unknown ? Program.MyHero.Spellbook.GetSpell(Slot) : null; }
        }
        ...
        public GameObject Source
        {
            get { return SourceObjectIsValid ? SourceObject : Program.MyHero; }
        }
        ...
        {
            get { return RangeCheckSourceObjectIsValid ? RangeCheckSourceObject : Program.MyHero; }
        }
        ...
        public float GetDamage(Obj_AI_Base target)
        {
            ...
            {
                _cachedDamage[target] = Program.CurrentChampion.GetSpellDamage(Slot, target);
            }
            ...
            switch (Type)
            {
                case SpellType.Targeted:
                    return RangeCheckSourceObject.InRange(target,
                        Range + Program.MyHero.BoundingRadius + target.BoundingRadius - 65);
        ...
        {
            if (!IsReady || Chat.IsOpen || Program.MyHero.Spellbook.IsCastingSpell || !InRange(target))
            {
                return;
            }
            ...
            if (Type == SpellType.Linear || Type == SpellType.Circular || Type == SpellType.Cone)
            {
                ...
                if (pred.HitChancePercent >= HitChancePercent)
                {
                    ...
                    if (Program.MyHero.Spellbook.CastSpell(Slot, pred.CastPosition))
                    {
            ...
            else if (Type == SpellType.Targeted)
            {
                ...
                if (Program.MyHero.Spellbook.CastSpell(Slot, target))
                {
                    LastSentTime = Core.GameTickCount;
                }
            }
            else if (Type == SpellType.Self)
            {
                ...
                if (pred.HitChancePercent >= HitChancePercent)
                {
                    ...
                    if (Program.MyHero.Spellbook.CastSpell(Slot))
                    {
                        LastSentTime = Core.GameTickCount;
                    }
                }
        ...
        {
            if (!IsReady || Chat.IsOpen || Program.MyHero.Spellbook.IsCastingSpell)
            {
                return;
            }
            if (ModeManager.LaneClear || ModeManager.LastHit || ModeManager.Harass)
            ...
            if (Program.MyHero.Spellbook.CastSpell(Slot, position))
            {
                LastSentTime = Core.GameTickCount;
            }
        }
