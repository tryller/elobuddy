        ...
        public static void Initialize()
        {
            DamageSources = new List<DamageSource>
            {
                new DamageSource
                {
                    ...
                    Damage =
                        target =>
                            Program.CurrentChampion.GetBestCombo(target).Damage - SpellManager.Ignite.GetDamage(target) -
                            SpellManager.Smite.GetDamage(target),
                    Color = Color.FromArgb(170, Color.DarkOrange),
                    Text =
                        target =>
                            (Program.CurrentChampion.GetBestCombo(target).Q ? "Q" : "") +
                            (Program.CurrentChampion.GetBestCombo(target).W ? "W" : "") +
                            (Program.CurrentChampion.GetBestCombo(target).E ? "E" : "") +
                            (Program.CurrentChampion.GetBestCombo(target).R ? "R" : "")
                },
                new DamageSource
                {
