...
namespace T7_Fizz 
{
    public static class SpellBlock
    {
        ...
        {
            if (!Program.check(Program.blocking,"BLOCK") || !Program.E.IsReady()) return;

            foreach (var skillshot in Evade.Evade.GetSkillshotsAboutToHit(Player.Instance, (int)(Program.E.CastDelay + Game.Ping / 2f)))
            {
                ...
                foreach (var spell in spells)
                {
                    ...
                    {
                        Program.E.Cast(Player.Instance.Position);
                    }
                }
            }
        }
