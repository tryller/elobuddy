            ...
            foreach (var target in EntityManager.Heroes.Enemies.Where(e => e.IsValidTarget(2000) && !e.IsDead))
     	    {
                ...
                if (useR && R.IsReady() && target.IsValidTarget(2000) && target.IsInRange(Player.Instance, 2000) && !target.IsInRange(Player.Instance, 800))
                {
                    ...
                    if (pred.CastPosition.CountEnemyChampionsInRange(R.Width) >= MinR && pred.HitChance >= HitChance.Medium)
                    {
...
// Thanks MarioGK has allowed me to use some his logic
		...
		public static void Stacks()
        {
            ...
            (!Player.Instance.IsInShopRange() && _Player.Position.CountEnemyChampionsInRange(2500) < 1 && !_Player.IsRecalling() && Player.Instance.ManaPercent >= mana && !EntityManager.MinionsAndMonsters.CombinedAttackable.Any(x => x.IsValidTarget(2000))
